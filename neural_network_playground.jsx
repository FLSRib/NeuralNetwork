...
export default function NeuralNetworkPlayground() {
  ...
  const [weightsState, setWeightsState] = useState(() => buildWeights([3, 4, 4, 2]));
  ...
  useEffect(() => {
    setInputs((prev) => Array.from({ length: inputSize }, (_, i) => (prev[i] !== undefined ? prev[i] : +(Math.random() * 2 - 1).toFixed(2))));
  }, [inputSize]);

  useEffect(() => {
    setWeightsState(buildWeights([inputSize, ...hiddenKey.split(',').map(Number), outputSize]));
    setSelected(null);
  }, [inputSize, hiddenKey]);
  ...
  if (selected) {
    if (selected.layer === 0) {
      inspector = (
        <div>
          <p className="text-sm font-mono text-cyan-300 mb-2">entrada · x{selected.idx + 1}</p>
          <p className="text-sm text-slate-300">
            valor = <span className="font-mono text-slate-100">{inputs[selected.idx].toFixed(3)}</span>
          </p>
          <p className="text-xs text-slate-500 mt-2">Valores de entrada não passam por cálculo — são o ponto de partida do sinal.</p>
        </div>
      );
    } else {
      const l = selected.layer - 1, i = selected.idx;
      const prevActs = activations[selected.layer - 1];
      const w = weightsState.weights[l][i];
      const b = weightsState.biases[l][i];
      const z = zs[l][i];
      const a = activations[selected.layer][i];
      inspector = (
        <div>
          <p className="text-sm font-mono text-cyan-300 mb-3">
            camada {selected.layer} · neurônio {i + 1}
          </p>
        ...
        <div className="flex flex-col lg:flex-row gap-6 mt-6">
          ...
          <aside className="w-full lg:w-72 shrink-0 space-y-5">
            <div className="rounded-lg border border-slate-800 bg-slate-900/60 p-4">
              ...
              <div className="flex items-center justify-between mb-3">
                ...
                <div className="flex items-center gap-2">
                  <button className={btnBase} onClick={() => setInputSize((s) => Math.max(2, s - 1))}>−</button>
                  <span className="w-5 text-center font-mono text-cyan-300 text-sm">{inputSize}</span>
                  <button className={btnBase} onClick={() => setInputSize((s) => Math.min(4, s + 1))}>+</button>
                </div>
              </div>
            ...
            <div className="flex flex-col gap-2">
              <button
                onClick={() => setWeightsState(buildWeights(layerSizes))}
                ...
                {weightsState.weights.map((wMatrix, l) =>
                  wMatrix.map((row, i) =>
                    row.map((w, j) => {
                      ...
                      const isIncoming = selected && selected.layer === l + 1 && selected.idx === i;
                      const opacity = selected ? (isIncoming ? baseOpacity : baseOpacity * 0.12) : baseOpacity;
                      return (
                        <line
                          key={`e-${l}-${i}-${j}`}
                ...
                {layerSizes.map((n, li) => (
                  <g key={`layer-${li}`}>
                    {Array.from({ length: n }).map((_, ni) => {
                      ...
                      const nodeRange = li === 0 ? [-1, 1] : range;
                      const fill = activationColor(value, nodeRange);
                      const isSelected = selected && selected.layer === li && selected.idx === ni;
                      return (
